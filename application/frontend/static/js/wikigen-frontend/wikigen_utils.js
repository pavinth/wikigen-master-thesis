                ...
                for (var anchor in jsonData) {
                    if (anchor !== "count") {
                        ...
                        var category = "<div id='category-form'> " +
                            "<input type='text' list='category-list' id='category-input' size='35' style='line-height:1.8;position:relative;top:17px;border:1px solid black;" +
                            "border-radius:4px;' name='category'>\n" +
                        ...
                        if (recalculatedDaysSurvived > 1) { // filter 0 values
                            ...
                            row = [];
                            row.push("<a href='#anchor_chronology_part'  style=font-size:18px; class='anchor_element'>" + anchor + "</a>");
...
function constructColumnsFromAnchorCompareData(array, anchorYearSnapshots) {
    ...
    for (var i = firstYear; i <= lastYear; i++) {
        resultColumns.push({"sTitle": "Strength in " + i});
        j++;
    }
