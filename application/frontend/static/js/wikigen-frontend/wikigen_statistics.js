...
function updateEditProgressbar() {
    var setPBValue = (editAnalysisCounter + talkEditAnalysisCounter) / (revisionCount) * 100;
...
function precalculateRevisionCountProcedure(title, nextPartPointer, newNextPartPointer, callbackSuccess, callbackError, articleType) {
    var url =
        wikiAPIEndPoint
        + actionQuery
        + propRevisions
        ...
        function (data) { // success in recieving data
            checkForNextBulk(data, "revisions", "rvcontinue",
                function (newNextPartPointer) { // More data
                    ...
                    precalculateRevisionCountProcedure(title, nextPartPointer, newNextPartPointer, callbackSuccess, callbackError, articleType);
                },
...
function precalculateRevisionCount(title, callbackSuccess, callbackError) {
    ...
    // for article page
    precalculateRevisionCountProcedure(title, "", "", successCallback, errorCallback, '');

    // for talk page
    precalculateRevisionCountProcedure('Talk:' + title, "", "", successCallback, errorCallback, 'talk');
}
...
function addUpToTalkStats(year, statType, month) {
    if (!statType[year]) {
        statType[year] = {"count": 1};
        statType[year][month] = 1;
    } else {
...
function addUpToTalkEditorsStats(user, year, statType, month) {
    var savedUser;
    var savedYearCounter;
    var savedMonthCounter;
    if (!statType[year]) {
...
function getTalkUserIsABot(user) {
    var isABot = botUsers[user];
    if (typeof (isABot) == 'undefined') {
        isABot = false;
    }
...
function getTalkUserIsAnonym(user) {
    return fnValidateIPAddress(user);
}
...
function talkUserPartOfEditStatistics(user, year, month, callback) {
    var isABot = getTalkUserIsABot(user);
    var isAnonym = fnValidateIPAddress(user);
    addUpToTalkEditorsStats(user, year, talkEditorsData, month);
    if (isABot) {
        addUpToTalkEditorsStats(user, year, botTalkEditorsData, month);
        addUpToTalkEditorsStats(user, year, registeredTalkEditorsData, month);
    } else {
        if (isAnonym) {
            addUpToTalkEditorsStats(user, year, anonymTalkEditorsData, month);
        } else {
            addUpToTalkEditorsStats(user, year, registeredTalkEditorsData, month);
        }
    }
...
function parseAndSaveTalkBots(data, callbackSuccess, callbackError) {
    //console.log('parseBulkForOverallEditStats');
    if (data && data.query && data.query.allusers) {
        var allusers = data.query.allusers;
        var botName;
        ...
        for (var id in pages) { // look for all pages
            ...
            for (var revid in revisions) { // look for all revisions of the page
                ...
                if (date < fromDate || date > untilDate) {
                    ...
                    if (lastRev && lastRev.user !== editor) { // we have a new user -> LAST revision is a distinct one
                        ...
                        // Do edits part
                        addUpToTalkStats(lastYear, distinctTalkEditData, lastMonth);
                        if (lastMajChg > 0) { // is a major change since there was at least one major change in the sequence
                            //console.log('found a major revision');
                            addUpToTalkStats(lastYear, distinctMajorTalkEditData, lastMonth);
                        } else { // it was not a major change!
                            //console.log('Not a major change');
                        }
                        talkUserPartOfEditStatistics(lastEditor, lastYear, lastMonth,
                            function (isABot, isAnonym) {
                                if (!isABot) {	// not a bot
                                    addUpToTalkStats(lastYear, nonBotDistinctTalkEditData, lastMonth);
                                    if (lastMajChg > 0) { // is a major change since there was at least one major change in the sequence
                                        //console.log('found a major revision');
                                        addUpToTalkStats(lastYear, nonBotDistinctMajorTalkEditData, lastMonth);
                                        addUpToTalkEditorsStats(lastEditor, lastYear, nonBotDistinctMajorTalkEditorsData, lastMonth);
                                    } else { // it was not a major change!
                                        //console.log('Not a major change');
                                    }
                                    if (isAnonym) { // anonymous
                                        ...
                                        //addUpToStats(lastYear, registeredDistinctEditData, lastMonth);
                                        if (lastMajChg > 0) {
                                            addUpToTalkStats(lastYear, registeredNonBotDistinctMajorTalkEditData, lastMonth);
                                            addUpToTalkEditorsStats(lastEditor, lastYear, registeredNonBotDistinctMajorTalkEditorsData, lastMonth);
                                        }
                                    }
                                }
                            }
                    ...
                    year = date.getFullYear();
                    month = date.getMonth();
                    addUpToTalkStats(year, overallTalkEditData, month);
                    lastRev = currentRevision;
                    if (typeof (editor) == 'undefined') {
                        //console.log("found ya!");
                    }
                    talkUserPartOfEditStatistics(editor, year, month,
                        function (isABot, isAnonym) {
                            if (!isABot) {	// not a bot
                                addUpToTalkStats(year, nonBotTalkEditData, month);
                                if (isAnonym) { // anonymous
                                    addUpToTalkStats(year, anonymousTalkEditData, month);
                                } else { // registered nonbot
                                    addUpToTalkStats(lastYear, registeredTalkEditData, month);
                                }
                            }
                        }
                    );
            ...
            if (lastDate < fromDate || lastDate > untilDate) {
                ...
                if (iterations < maxResults) {
                    ...
                    if (!getTalkUserIsABot(lastEditor)) {	// not a bot
                        addUpToTalkStats(lastYear, nonBotDistinctTalkEditData, lastMonth);
                        if (lastMajChg > 0) { // is a major change since there was at least one major change in the sequence
                            //console.log('found a major revision');
                            addUpToTalkStats(lastYear, nonBotDistinctMajorTalkEditData, lastMonth);
                        ...
                        if (getTalkUserIsAnonym(lastEditor)) { // anonymous
                            // see above
                            //addUpToStats(lastYear, anonymousDistinctEditData, lastMonth);
                            if (lastMajChg > 0) {
                                // see above
                            ...
                            addUpToTalkStats(lastYear, registeredDistinctTalkEditData);
                            if (lastMajChg > 0) {
                                addUpToTalkStats(lastYear, registeredNonBotDistinctMajorTalkEditData, lastMonth);
                                addUpToTalkEditorsStats(lastEditor, lastYear, registeredNonBotDistinctMajorTalkEditorsData, lastMonth);
                            }
...
function saveBacklinkDataForReferenceStats(backlink, year, type, lastType) {
    ...
    var row = [
        "<a href='http://" + wikiLang + ".wikipedia.org/wiki/" +
        backlink +
        "'" +
        "target=_'blank'" +
        ">" +
        backlink +
        "</a>: ", year, lastType, type];
